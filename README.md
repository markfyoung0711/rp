...
- **Personal data in output:** only the first name (in the greeting) and your own `task_id`, plus, in resident renewal messages, the resident's own unit (in the wording and the renewal link; reported as `meta.used_on_purpose`). Nothing else from the profile ever appears, in any field; see `tests/pii_cases.jsonl`.
- **Stage playbook:** stages the samples never showed (no-show, renewal window / undecided / details, resident welcome, loyalty) get their CTA, link, reply options and next action from `config/rules.yaml` (`default_cta_by_stage`, `cta`, `next_action.by_stage`) and their wording from `config/templates/en.yaml`. Each rule comes from one hold-out example, so it's fitted, not learned. Other languages use their general line until they get wording.
